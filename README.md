...
This example shows how to package an algorithm for use with SageMaker. We have chosen a simple [scikit-learn][skl] implementation of logistic regression to illustrate the procedure.

[Here][aws_sagemaker_examples] you find other examples using SageMaker.
...
* __logistic_regression__: The directory that contains the application to run in the container. See the next session for details about each of the files.

* __local_test__: A directory containing scripts and a setup for running a simple training and inference jobs locally so that you can test that everything is set up correctly. See below for details.
...
[aws_sagemaker_examples]: https://github.com/aws/amazon-sagemaker-examples/tree/master/advanced_functionality/scikit_bring_your_own "Amazon SageMaker Examples"